package com.bme.opentsdb.client.tsdb.http;
...
public class HttpClientFactory {
    ...
    private static Map<String, HttpClient> httpClientMap = new ConcurrentHashMap<>();
    ...
    public static HttpClient createHttpClient(OpenTSDBConfig config) throws IOReactorException {
        if (!httpClientMap.containsKey(config.getHost() + config.getPort())) {
            synchronized (HttpClientFactory.class) {
                if (!httpClientMap.containsKey(config.getHost() + config.getPort())) {
                    ...
                    /**初始化请求配置
                     * 特别注意connectionRequestTimeout,socketTimeout在其中设置的，
                     */
                    ...
                    HttpClient httpClient = new HttpClient(config, httpAsyncClient, initFixedCycleCloseConnection(manager));
                    httpClientMap.put(config.getHost() + config.getPort(), httpClient);
                    httpClient.start();
                    return httpClient;
                }
            }
        }
        return httpClientMap.get(config.getHost() + config.getPort());
    }
